...
export async function summarizePage(page: PageContext): Promise<string> {
...
export async function tasksFromPage(
  goal: SylviaGoal,
  page: PageContext
): Promise<SylviaTask[]> {
  const messages: LlmMessage[] = [
...
${page.content}`
  ...
  try {
    ...
    const tasks = (parsed.tasks ?? []) as SylviaTask[];
...
export async function conceptFromPage(
  page: PageContext
...
export async function chatWithSylvia(
  req: SylviaChatRequest
): Promise<string> {
  const messages: LlmMessage[] = [
    { role: "system", content: SYSTEM_PROMPT }
  ];
