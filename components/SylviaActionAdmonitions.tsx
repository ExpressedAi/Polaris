...
const SylviaActionAdmonitions: React.FC<Props> = ({ events, onRate }) => {
  ...
  const handleDismiss = async (id: string) => {
    ...
    await saveSetting('dismissedActionAdmonitions', Array.from(newDismissed));
  };
    ...
    <div className="space-y-3 px-5 py-4">
      {visibleEvents.map(event => {
        ...
        return (
          <div
            key={event.id}
            className={`admonition admonition-${type} relative group`}
          >
            {/* Dismiss button */}
            <button
...
export default SylviaActionAdmonitions;
